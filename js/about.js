function parseTweets(runkeeper_tweets) {
    ...
    ;[...document.getElementsByClassName("liveEventsPct")].forEach((element) => {
        element.innerText =
            math.format(
                (tweet_array.filter((tweet) => tweet.source === "live_event").length /
                    tweet_array.length) *
                    100,
                {
                    precision: 2,
    ...
    ;[...document.getElementsByClassName("achievementsPct")].forEach((element) => {
        element.innerText =
            math.format(
                (tweet_array.filter((tweet) => tweet.source === "achievement").length /
                    tweet_array.length) *
                    100,
                {
                    precision: 2,
    ...
    ;[...document.getElementsByClassName("miscellaneousPct")].forEach((element) => {
        element.innerText =
            math.format(
                (tweet_array.filter((tweet) => tweet.source === "miscellaneous").length /
                    tweet_array.length) *
                    100,
                {
                    precision: 2,
